...
type
  ...
  TExecutableFile = class
   ...
   public
    ...
    function SaveToFile(var f: TextFile; a:TMemoryStream;  SaveOptions: TSaveOptions): boolean; overload; virtual;
    function SaveToFile(DHF: TFileStream; var DAS: TextFile; SaveOptions: TSaveOptions): boolean; overload; virtual;
...
function TExecutableFile.SaveToFile(var f: TextFile; a:TMemoryStream;  SaveOptions: TSaveOptions):boolean;
var
  i:integer;
...
begin
  ...
  if soProject in SaveOptions then begin
    ...
    a.Write(StreamSectionCount, 4);
    for i:=0 to Sections.Count-1 do begin
      a.Write(Sections[i].typ, sizeof(TSectionType));
      if not Sections[i].SaveToFile(f, a, SaveOptions) then Exit;
    end;
  end
  // Save disassembled code sections
    ...
    for i:=0 to Sections.Count-1 do
      if Sections[i].typ = stCode then
        if not Sections[i].SaveToFile(f,a,SaveOptions) then
          Exit;
...
function TExecutableFile.SaveToFile(DHF: TFileStream; var DAS: TextFile; SaveOptions: TSaveOptions): boolean;
var
...
begin
  ...
  if soProject in SaveOptions then begin
    ...
    for SectionIndex:=0 to Sections.Count-1 do begin
      ...
      if not Sections[SectionIndex].SaveToFile(DAS, DHF, SaveOptions) then Exit;
    end;
  end
  ...
  // Save disassembled code sections
    ...
    for SectionIndex:=0 to Sections.Count-1 do
      if Sections[SectionIndex].typ = stCode then
        if not Sections[SectionIndex].SaveToFile(DAS, DHF, SaveOptions) then
          Exit;
